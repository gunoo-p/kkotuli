...
| 기술 | 선택 이유 |
|------|-----------|
| **React 19.2.6** | 컴포넌트 기반 UI 구성, Vite 번들러로 빠른 개발 환경 |
| **Kotlin 2.0.21** | `data class`로 DTO 보일러플레이트 제거, Null Safety로 런타임 NPE 방지, Spring Boot 3.x 공식 지원. Java 대비 코드량 약 30~40% 감소 |
...
| **Redis 7-alpine** | 인메모리 구조로 게임 진행 중 단어 검증 · 턴 상태를 밀리초 단위 처리. TTL로 게임 종료 후 자동 소멸. MySQL 불필요 (영구 저장 데이터 없음) |
